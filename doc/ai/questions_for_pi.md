...
## Tar-шарды библиотек орбит (launch_orblib_exp.sh, эксперимент orblib_exp)

17. **Политика консолидации шардов.** Каждый запуск `launch_orblib_exp.sh`
    заливает на Я.Диск свой уникальный tar-шард
    `orblib_{KEY}__{host}_{timestamp}.tar`; со временем шардов станет много
    (медленнее ШАГ 0 pre-download). Когда и кто сливает шарды в один
    (порог по числу/размеру? вручную оффлайн? отдельный скрипт с удалением
    исходных шардов после проверки)?

Ответ:

18. **Хранить ли `ic` (и `inttime`) в `.npz`?** При reuse они не нужны:
    после `agama.orbit` ни `ic`, ни `inttime` не используются в
    solve/penalty/Upsilon. Хранение — только для воспроизводимости /
    возможной переинтеграции орбит. `ic` — ~4.8 MB float64 на 100k орбит
    (плюс `inttime`); отказ от них заметно уменьшит шарды. Убрать из `.npz`?

Ответ:

19. **Формат имени шарда.** Сейчас `{HOSTNAME_ENV}_{TIMESTAMP}` =
    `$(hostname)_YYYYmmdd_HHMMSS` (как в `launch_docker_parallel.sh`).
    Подтвердить, что hostname в имени шарда должен совпадать с
    `HOSTNAME_ENV`, и устраивает ли формат timestamp.

Ответ:

20. **Найденный баг (для информации; исправлен только в `launch_orblib_exp.sh`,
    прод НЕ трогали).** Реальный прогон `launch_orblib_exp.sh --incl=90.0
    --no-double` привёл к падению всех контейнеров, при этом ни один лог не
    залился на Я.Диск и не пришло ни одного уведомления. Причина, воспроизведена
    локально мок-тестом (docker/rclone/curl подменены, без реальных вычислений):
    строка `DONE_COUNT=$(ls .../.done_orblib_* 2>/dev/null | wc -l)` под
    `set -euo pipefail` — если НИ ОДИН контейнер не создал `.done_orblib_*`
    (все упали), `ls` возвращает ненулевой код (glob не раскрылся), `pipefail`
    делает статус всего конвейера ненулевым, и `set -e` **аварийно валит весь
    скрипт на этой строке**, минуя финальное объединение, заливку шарда
    орблибов, финальную синхронизацию истории и заливку мастер-лога, и
    финальное summary-уведомление. EXIT-trap успевает отправить только одно
    аварийное уведомление и не всегда — заливка LOGFILE в нём выполняется
    только если `sudo shutdown` разрешён (в `--no-shutdown`-режиме тоже
    исправлено).
    В `launch_orblib_exp.sh` исправлено: `DONE_COUNT` считается из уже
    отслеженного `FAILED` (`N_PROC - FAILED`), без обращения к файловой
    системе; добавлены немедленные ntfy-уведомления при падении КАЖДОГО
    контейнера (не только итоговое summary); мастер-лог заливается на
    Я.Диск сразу после ШАГ 3 (а не только в конце); `notify()` теперь
    проверяет код возврата `curl` (`-f`) и пишет в лог при неудаче отправки;
    устранена гонка с дублирующим аварийным уведомлением при `--no-shutdown`.
    **ВАЖНО:** точно такая же конструкция (`ls .../.done_* | wc -l` под
    `set -euo pipefail`) есть и в проде `launch_docker_parallel.sh` — тот же
    латентный баг сработает там при полном падении всех контейнеров. Прод
    НЕ менял (вне рамок задачи и запрещено правилами) — нужно подтверждение
    и отдельная задача на исправление, если это релевантно.
